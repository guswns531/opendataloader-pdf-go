...
To efciently scale-up LLMs, we aim to utilize pre-
...
sources while introducing novel modications to
...
Depthwise scaling.From the base model withn
  ...
  - e.,from n to2n layers. Then, the `layer distance',
...
at the seam, which may be too signicant of a

duplicated for subsequent modication. Then, we

discrepancy for continued pretraining to quickly

remove the nalm layers from the original model

resolve. Instead, depthwise scaling sacrices the
