...
the instruction-tuned model is further ne-tuned to
...
scaled model is sufficiently contained before the
  ...
  1. Similar to the instruction tuning stage, we
...
rephrased question is a better answer than the orig- inal answer, possibly due to the interim rephrasing
  ...
  1. Training Details step. Consequently, we set the rephrased question as the prompt and use the rephrased answer as the
...
perform ne-tuning of SOLAR 10.7B in two stages:
...
Open Source O O	7	O O	7

open-source datasets but also synthesize a math QA4 Results dataset to enhance the model's mathematical capa-
