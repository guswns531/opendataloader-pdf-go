...
We reformatted the instruction datasets with ansmaller size, SOLAR 10.7B-Instruct scores the

Alpaca-styled chat template. For datasets such ashighest in terms of H6, even surpassing the recent OpenOrca, which are derived from FLAN (Long-top-performing open-source LLM Mixtral 8x7B- pre et al., 2023), we lter data that overlaps withInstruct-v0.1 or Qwen 72B. The above results indi- the benchmark datasets (see Tab.  8 in Appendix.cate DUS can up-scale models that are capable of C for more information). The alignment datasets areachieving state-of-the-art performance when ne- in the {prompt, chosen, rejected} triplet format.tuned. We also report data contamination results We preprocess the alignment datasets followingfor SOLAR 10.7B-Instruct in Appendix C. Zephyr (Tunstall et al., 2023).
...
models are prefixed with SFT for supervised ne-
...
OpenOrca affect the trained models. The rst ab- lated model, `SFT v1', which used only the Alpaca-
...
Mixtral 8x7B-Instruct-v0.147B Instruction-tuned 72.62 70.22 87.63 71.16 64.58 81.37 60.73
  ...
  1. 7B and SOLAR 10.7B-Instruct models along
...
Falcon 180B	180B Pretrained 67.85 69.4588.86 70.50 45.47	86.90 45.94
...
of57:32 compared to52:24 of `SFT v1' but also
