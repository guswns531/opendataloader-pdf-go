...
Table 7: Ablation studies on the different merge methods used for obtaining the nal model. We use `Cand. 1' and `Cand. 2' from Tab. 6 as our two models for merging. We name the merged models with the `Merge' prefix to indicate they are merged. The best scores for H6 and the individual tasks are shown in bold.

tively impacted by adding Synth. Math-Alignment.To utilize this for the alignment-tuned model as Thus, we can conclude that adding Synth. Math-well, we train two models named `Cand. 1' and Alignment is beneficial for H6.	`Cand. 2' using the same training dataset and SFT Then, we experiment whether merging `DPO base model as `DPO v2' and `DPO v3' but with dif- v1' and `DPO v2' is beneficial. Unfortunately,ferent hyper-parameters to maximize each model's `DPO v1+v2' scores73:21 in H6, which is worserespective strengths. We compare `Cand. 1' and than `DPO v2'. More importantly, the gain in`Cand. 2' in Tab.  6 where we can see that `Cand. 1' the GSM8K score from adding Synth. Math-	has high GSM8K scores but relatively low scores Alignment is gone, which is undesirable. Onefor the other tasks, whereas `Cand. 2' has low reason for this could be that `DPO v2' is a strictscores for GSM8K but high scores for the other improvement over `DPO v1', unlike the case fortasks. We merge these two models using various merging `SFT v3' and `SFT v4' where the modelsmethods and ablate the results in Tab.. 7. had different strengths and weaknesses.	We use two merge methods: 1) Average (a,b),
...
Cand. 173.73 70.48 87.47 65.73 70.62 81.53	66.57

ing that as long as the merge candidates have suf-

Cand. 2 73.2871.59 88.39 66.14 72.50 81.99	59.14
...
tasks compared to `SFT v3', and the gap is espe- cially large for ARC (+1 :45) and GSM8K (+2 :43).
...
Surprisingly, the two models perform similarly in Merge v1 Average (0.5, 0.5) 74.0071.16 88.01 66.14 71.71	82.08 64.90

We introduce SOLAR 10.7B and its ne-tuned vari-

Merge v2 Average (0.4, 0.6) 73.93 71.0888.08 66.27 71.89	81.77 64.52 terms of H6. A closer look at the scores for the

Merge v3 Average (0.6, 0.4)74.05 71.08 87.88 66.13 71.61 ant SOLAR 10.7B-Instruct, which are depth up-82.08 65.50

individual tasks shows only a small margin in the Merge v4 SLERP 73.96	71.16 88.03 66.25 71.79 81.93 64.59
...
Tab. 3, we saw that merging two models that havemore exploration, DUS could be further improved,